...
(defmacro defgpuconstant (name value &optional doc)
...
(defmacro defgpuenum (name &body enum-list)
...
(defmacro defgputype (name base-type &optional documentation)
  `(eval-when (:compile-toplevel :load-toplevel :execute)
...
(defmacro defgpustruct (name &body fields)
...
(defmacro defgpuunion (name &body fields)
...
(defmacro defgpufun ((c-name lisp-name) return-type &body args)
  ...
  `(eval-when (:compile-toplevel :load-toplevel :execute)
     (cffi:defcfun (,c-name ,lisp-name :library libsdl2-gpu :convention :stdcall) ,return-type
       ,@args)
...
(defmacro defgpu-lispfun (name lambda-list &body body)
  "Wrapper around `defun' which additionally exports the function name.
Meant to be used around SDL_gpu C preprocessor macros which have to be implemented as lisp functions."
...
(defmacro fdeclgpu (struct-or-union)
